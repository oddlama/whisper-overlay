...
pub fn get_window_x11<T: IsA<gtk::Window>>(
    window: &T,
) -> Option<(gdk_x11::X11Display, gdk_x11::X11Surface)> {
    let surface = window
        .upcast_ref()
        .surface()?
        .downcast::<gdk_x11::X11Surface>()
        .ok()?;
    let display = surface.display().downcast::<gdk_x11::X11Display>().ok()?;
...
impl Atom {
    pub fn new(display: &gdk_x11::X11Display, prop: &str) -> Result<Self, NulError> {
        unsafe {
            let prop = CString::new(prop)?;
            Ok(Self(gdk_x11::ffi::gdk_x11_get_xatom_by_name_for_display(
...
pub unsafe fn change_property<T: XProp>(
    display: &gdk_x11::X11Display,
    surface: &gdk_x11::X11Surface,
    prop: &str,
    mode: PropMode,
    value: T,
) {
